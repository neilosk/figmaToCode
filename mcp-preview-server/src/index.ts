 ...
 */
async function handleRenderComponentPreview(args: any): Promise<CallToolResult> {
  const {
    code,
    componentName,
    framework = 'react',
 ...
 */
async function handleGetPreviewStatus(args: any): Promise<CallToolResult> {
 ...
 */
async function handleGetPreviewUrl(args: any): Promise<CallToolResult> {
  const { previewId } = args;
 ...
 */
async function handleAnalyzeCodeComplexity(args: any): Promise<CallToolResult> {
  const { code } = args;
 ...
 */
function createArtifactResponse(previewId: string, componentName: string, focusedPrompt: string, chunks: any[]): string {
